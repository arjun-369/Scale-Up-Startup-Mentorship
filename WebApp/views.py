...
from WebApp.models import Mentor, Startup
...
def SuggestionAction(request):
    if request.method == 'POST':
        ...
        mentor_mail = None
        db_connection =  pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'scaleup',charset='utf8')
...
def RatingAction(request):
    if request.method == 'POST':
        ...
        db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'mentor',charset='utf8')
        db_cursor = db_connection.cursor()
...
def MentorRegisterAction(request):
    if request.method == 'POST':
        ...
        with con:
            cur = con.cursor()
            cur.execute("select email from mentorregister where email = '"+email+"'")
            rows = cur.fetchall()
            for row in rows:
                if row[0] == email:
        ...
        if status == 'none':
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'mentor',charset='utf8')
            db_cursor = db_connection.cursor()
            student_sql_query = "INSERT INTO mentorregister(first_name,last_name,email,contact_no,address,age,password,qualification,experience) VALUES('"+fname+"','"+lname+"','"+email+"','"+contact+"','"+address+"','"+age+"','"+password+"','"+qualification+"','"+experience+"')"
            db_cursor.execute(student_sql_query)
            db_connection.commit()
            print(db_cursor.rowcount, "Record Inserted")
            if db_cursor.rowcount == 1:
